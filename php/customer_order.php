...
if(isset($_POST["logout"]))
	...
	//}
   $sql1 = "SELECT bill.bill_id, bill.bill_date,bill_product.pharmacy_id,ver_pharmacy.pharmacy_name, bill_product.prod_id,product.product_name,bill_product.status from `bill_product` INNER JOIN `bill` ON bill.customer_id=".$_SESSION['cust_id']." INNER JOIN `product` ON product.product_id=bill_product.prod_id INNER JOIN `ver_pharmacy` ON bill_product.pharmacy_id=ver_pharmacy.pharmacy_id WHERE bill_product.bill_id=bill.bill_id";
$result = $conn->query($sql1);
  ...
  if (isset($_POST['cancel'])){ 
       ...
       $sql ="UPDATE `bill_product` SET `status` = 'Customer Cancelled' WHERE `bill_product`.`bill_id` = ".$_POST['bill_id']." AND `bill_product`.`prod_id` = ".$_POST['prod_id'] ;
...
<div id="sidebar">
  ...
  <h2>CUSTOMER PROFILE</h2>
    ...
    <div class="infront">
    	<h3><a href="..\php\customer_profile.php">PROFILE </a></h3>
		        <h3><a href="..\php\customer_order.php">ORDER</a></h3>

		<h3><a href="..\php\customer_profile_app_confir.php">APPOINTMENT </a></h3>
<h3><a href="..\php\review.php">GIVE REVIEW </a></h3>
		 </form>
...
echo'<div class="cart_date"><b><br><center>'.$row['bill_date'].'<br>Bill Id:'.$row['bill_id'].'</center></b></div>';
echo'<div class="cart_name"><b>Pharmacy Name:</b>'.$row['pharmacy_name'].'<br><b>Product Name</b>'.$row['product_name'].'<br><b>Status:</b>'.$row['status'].'<br></div>';
echo"<div class='cart_del'>  <form action='#' method='POST'><input type='hidden' name='bill_id' value='".$row['bill_id']."'><input type='hidden' name='prod_id' value='".$row['prod_id']."'><input type='submit' name='cancel' value='Cancel' style='font-family:Verdana, sans-serif;font-weight:bold;font-size:14 pt;color:black;background-color:white;border:2px solid #000000;padding:1px;height:30px;width:150px;'></form></div>";
