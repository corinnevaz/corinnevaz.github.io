...
exit();
   ...
   {
	   $sql1='UPDATE appointment SET Status="Cancelled by customer" WHERE appointment_id='.$query;  
   $res1 = mysqli_query($conn, $sql1);
   
	if(mysqli_query($conn,$sql1)){
	header("location: customer_profile_app_confir.php");
	exit();
		...
		$sql = "SELECT * FROM appointment where customer_id=".$_SESSION['cust_id'];
...
echo'<div class="cart_name"><b>Doctor Name:</b>'.$row['doctor_name'].'<br><b>Time:</b>'.$row['a_time'].'<br><b>Address:</b>'.$row['doctor_ad'].'<br><b>Staus:</b>'.$row['Status'].'</div>';


echo'<div class="cart_del">  <button class="buttoncus" ><form action="#" method="POST"><input type="hidden" name="app_id" value="'.$row['appointment_id'].'"><input type="submit" value="Cancel" name="cancel" style="text-decoration:none;border:none;background-color:white;"></form></div></div>';
