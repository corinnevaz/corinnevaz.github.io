...
if(isset($_POST["logout"]))
    ...
    if (isset($_POST['cancel'])){ 
        ...
        $sql = 'SELECT * FROM appointment WHERE appointment_id='.$query;
		$res = mysqli_query($conn, $sql);
		if (mysqli_num_rows($res) > 0) {
    // output data of each row
   while($row = mysqli_fetch_assoc($res))
   {
	   $sql1='UPDATE appointment SET Status="Cancelled by Doctor" WHERE appointment_id='.$query;  
   $res1 = mysqli_query($conn, $sql1);
   
	if(mysqli_query($conn,$sql1)){
	header("location: doctor_profile_appconfir.php");
	exit();
		...
		$sql = "SELECT * FROM appointment where Status ='Approval Pending' and doctor_id=".$_SESSION['doc_id'];
...
if (isset($_POST['accept'])){ 
   ...
   while($row = mysqli_fetch_assoc($res))
   {
	   $sql1='UPDATE `appointment` SET `Status`="Accepted" WHERE appointment_id='.$query;  
   $res1 = mysqli_query($conn, $sql1);
...
echo'<div class="cart_date"><b><br><center>'.$row['a_date'].'</center></b></div>';
echo'<div class="cart_name"><b>Name: </b>'.$row['customer_name'].'<br><b>Time: </b>'.$row['a_time'].'<br><b>Mobile: </b>'.$row['customer_mobile'].'<br><b>Status: </b>'.$row['Status'].'</div>';

echo'<div class="cart_del"> <button class="button" ><form action="#" method="POST"><input type="hidden" name="app_id" value="'.$row['appointment_id'].'"><input type="submit" value="Accept" name="accept" style="text-decoration:none;border:none;background-color:white;"></form>';
echo' <button class="button" ><form action="#" method="POST"><input type="hidden" name="app_id" value="'.$row['appointment_id'].'"><input type="submit" value="Cancel" name="cancel" style="text-decoration:none;border:none;background-color:white;"></form></div></div>';
