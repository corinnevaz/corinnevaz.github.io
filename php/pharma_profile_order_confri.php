...
exit();
  ...
  $sql2 = "SELECT bill_product.pharmacy_id,bill_product.bill_id,bill.bill_date,bill_product.prod_id,product.product_name,bill_product.status FROM bill_product INNER JOIN bill ON bill.bill_id=bill_product.bill_id INNER JOIN product ON product.product_id=bill_product.prod_id WHERE bill_product.pharmacy_id=".$_SESSION['pharma_id']." AND bill_product.status !='order recieved'";
...
echo'<div class="cart_date"><b><br><center>'.$row['bill_date'].'</center></b></div>';
echo'<div class="cart_name"><br><b>Bill Id:</b>'.$row['bill_id'].'<br><b>Pharmacy Id:</b>'.$row['pharmacy_id'].'<br><b>Product Name</b>'.$row['product_name'].'<br><b>Status:</b>'.$row['status'].'</div>';
echo"<div class='cart_del'>  <form  action='update3.php' method='POST'><input type='hidden' name='bill_id' value='".$row['bill_id']."'><input type='hidden' name='prod_id' value='".$row['prod_id']."'><input type='submit' name='update' value='Update' style='font-family:Verdana, sans-serif;font-weight:bold;font-size:14 pt;color:black;background-color:white;border:2px solid #000000;padding:1px;height:30px;width:150px;'></form></div>";
