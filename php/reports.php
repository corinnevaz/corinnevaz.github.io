...
$sql ="SELECT appointment.doctor_id, appointment.doctor_name, COUNT(appointment.doctor_id) FROM appointment GROUP BY appointment.doctor_id";
...
echo "<table border=1px height=auto width=100% style='background-color:#F5F5F5; background-image:url('shell.jpg');'>";
echo" <tr>";
echo"<td colspan=5><center>MOST APPOINTED DOCTOR AS ON:" .date("m-d-y").  "</td>";
echo "</tr>";
echo" <tr >";
echo "<td>DOCTOR ID </td>";
echo" <td>DOCTOR NAME</td>";
echo" <td>NUMBER OF TIMES APPOINTED </td>";
echo "</tr>";
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr> <td>". $row['doctor_id'] ."</td><td>". $row['doctor_name'] ."</td><td>". $row['COUNT(appointment.doctor_id)'] ."</td></tr>";
    }
} else {
    echo "0 results";
}
echo"</table>";
echo"<br>";
echo "<table border=1px height=auto width=100% style='background-color:#F5F5F5; '>";
echo" <tr>";
echo"<td colspan=5><center>MOST ORDERED MEDICINE AS ON:" .date("m-d-y").  "</td>";
echo "</tr>";
echo" <tr >";
echo "<td>PRODUCT ID </td>";
echo" <td>PRODUCT_NAME</td>";
echo" <td>NO. OF TIMES ORDERED </td>";
echo "</tr>";

$sql = "SELECT bill_product.prod_id, product.product_name,COUNT(bill_product.prod_id) FROM `bill_product` INNER JOIN `product` ON product.product_id= bill_product.prod_id  GROUP BY `prod_id`";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr> <td>". $row['prod_id'] ."</td><td>". $row['product_name'] ."</td><td>". $row['COUNT(bill_product.prod_id)'] ."</td></tr>";
    }
} else {
    echo "0 results";
}
